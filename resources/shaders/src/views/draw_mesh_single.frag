...
vec4 sample_atlas(vec2 tx, uint layer, bool sample_3f) {
  if (sample_3f) return texture(b_txtr_3f, vec3(tx, layer));
  else           return texture(b_txtr_1f, vec3(tx, layer));
}
...
vec4 sample_texture(uint texture_i, in vec2 tx_in) {
  ...
  vec2 tx = (vec2(info.offs) + vec2(info.size) * mod(tx_in, 1)) / wh;
