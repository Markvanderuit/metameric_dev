...
SurfaceInfo init_surface() {
  SurfaceInfo si;
  si.object_i = SURFACE_INVALID_DATA;
  return si;
}

bool is_valid(in SurfaceInfo si) {
  return si.object_i != SURFACE_INVALID_DATA;
}
...
SurfaceInfo get_surface_info(in Ray ray) {
  ...
  if (!is_valid(ray) || !hit_object(ray))
    return si;
...
vec3 surface_offset(in SurfaceInfo si, in vec3 d) {
  ...
  // float mag = M_RAY_EPS; // (1.f + hmax(abs(si.p))) * M_RAY_EPS;
  // mag = dot(si.n, d) >= 0 ? mag : -mag;
  // return fma(vec3(mag), si.n, si.p);

  // mag = dot(si.n, d) >= 0.f ? mag : -mag;
  // return fma(vec3(mag), si.n, si.p);

  // return si.p + si.n * 0.0005;

  // float mag = (1.f + hmax(abs(si.p))) * M_RAY_EPS;
  // return fma(vec3(mag), si.n, si.p);
}
