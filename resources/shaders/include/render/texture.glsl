...
vec3 si_to_coef_atlas_tx(in SurfaceInfo si) {
  // Load relevant info objects
  ObjectInfo object_info = scene_object_info(record_get_object(si.data));
  AtlasInfo  atlas_info  = scene_texture_coef_info(record_get_object(si.data));
...
vec4 texture_reflectance(in SurfaceInfo si, in vec4 wvls, in vec2 sample_2d) {
  ...
  vec3 tx = si_to_coef_atlas_tx(si);
  ...
  uvec4 pack = scene_texture_coef_fetch(ivec3(tx) + ivec3(tx_offsets[i], 0));
...
vec2 texture_brdf(in SurfaceInfo si, in vec2 sample_2d) {
  // Translate surface uv data to texture atlas uv
  vec3 tx = si_to_brdf_atlas_tx(si);
  ...
  uint pack = scene_texture_brdf_fetch(ivec3(tx) + ivec3(tx_offsets[i], 0));
