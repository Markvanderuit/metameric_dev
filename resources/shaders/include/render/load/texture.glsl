...
#define declare_scene_texture_data(scene_buff_coef_info_info,         \
                                   scene_buff_brdf_info_info,         \
                                   scene_txtr_coef_data,              \
                                   scene_txtr_brdf_data,              \
                                   scene_txtr_bsis_data)              \
  AtlasInfo scene_texture_coef_info(uint object_i) {                  \
    return scene_buff_coef_info_info[object_i];                       \
  }                                                                   \
  AtlasInfo scene_texture_brdf_info(uint object_i) {                  \
    return scene_buff_brdf_info_info[object_i];                       \
  }                                                                   \
                                                                      \
  ...
  uvec4 scene_texture_coef_fetch(ivec3 p) {                           \
    return floatBitsToUint(texelFetch(scene_txtr_coef_data, p, 0));   \
  }                                                                   \
  uint scene_texture_brdf_fetch(ivec3 p) {                            \
    return floatBitsToUint(texelFetch(scene_txtr_brdf_data, p, 0).x); \
  }                                                                   \
                                                                      \
