...
void init_brdf_dielectric(in Object object, inout BRDF brdf, in Interaction si, vec4 wvls, in vec2 sample_2d) {
...
BRDFSample sample_brdf_dielectric(in BRDF brdf, in vec3 sample_3d, in Interaction si) {
...
vec4 eval_brdf_dielectric(in BRDF brdf, in Interaction si, in vec3 wo) {
  // Compute eta for hero wavelength only, the rest gets killed later on
  // Compute fresnel, angle of transmission
  float eta = _brdf_eta_dispersive(brdf, wvls.x);
  float cos_theta_t;
