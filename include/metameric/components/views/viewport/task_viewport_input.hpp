...
namespace met {
  ...
  public:
    ...
    void init(detail::TaskInitInfo &info) override {
      ...
      // Local state
      m_gamut_prev = e_rgb_gamut; // Store a copy of the initial gamut as previous state
    ...
    void eval_select(detail::TaskEvalInfo &info) {
      ...
      if (io.MouseReleased[1]) {
        ...
        auto is_in_rect = std::views::filter([&](uint i) {
          eig::Array2f p = eig::window_space(e_rgb_gamut[i], i_arcball.full(), viewport_offs, viewport_size);
    ...
    void eval_gizmo(detail::TaskEvalInfo &info) {
      ...
      const auto gamut_selection = i_gamut_ind | std::views::transform(i_get(e_rgb_gamut));
      ...
      if (ImGuizmo::IsUsing()) {
        ...
        const auto move_selection = i_gamut_ind | std::views::transform(i_get(e_rgb_gamut));
