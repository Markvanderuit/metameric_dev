...
namespace met {
  ...
  struct MeshViewportViewBeginTask : public detail::TaskNode {
    ...
    void eval(SchedulerHandle &info) override {
      ...
      info("is_active").getw<bool>()
        = ImGui::Begin("Mesh Viewport", 0, ImGuiWindowFlags_NoBringToFrontOnFocus);
      ...
      // Note: window end is post-pended in MeshViewportViewEndTask
    }
  };
  
  struct MeshViewportViewEndTask : public detail::TaskNode {
    void eval(SchedulerHandle &info) override {
      met_trace_full();
      ...
      // Note: window end is post-pended here, but window begin is in MeshViewportViewBeginTask
      ImGui::End();
    }
  };

  class MeshViewportDrawBeginTask : public detail::TaskNode {
  ...
  struct MeshViewportDrawEndTask : public detail::TaskNode {
  ...
  void MeshViewportTask::init(SchedulerHandle &info) {
    met_trace_full();

    info.child_task("view_begin").init<MeshViewportViewBeginTask>();
