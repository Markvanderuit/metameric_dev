...
namespace met {
  ...
  bool ViewportOverlayTask::is_active(SchedulerHandle &info) {
    ...
    // on a single vertex
    return !info("viewport.input.vert", "selection").getr<std::vector<uint>>().empty();
  }
  ...
  void ViewportOverlayTask::eval(SchedulerHandle &info) {
    ...
    const auto &i_cstr_slct  = info.resource("constr_selection").getr<int>();
    ...
    if (e_vert_slct.size() > 1)
      info.resource("constr_selection").getw<int>() = -1;
    ...
    if (e_vert_slct.size() == 1 && i_cstr_slct >= 0) {
      ...
      if (ImGui::Begin(fmt::format("Editing {}", i_cstr_slct).c_str(), &window_open, window_flags)) {
    ...
    // Spawn window for reflectance plot display of given vertices
    if (!e_vert_slct.empty()) {
      view_size.y() = 0.f;
  ...
  void ViewportOverlayTask::eval_overlay_vertex(SchedulerHandle &info, uint i) {
    ...
    // Get external resources
    const auto &e_spec      = info.resource("gen_spectral_data", "spectra").getr<std::vector<Spec>>()[i];
    const auto &e_vert_slct = info.resource("viewport.input.vert", "selection").getr<std::vector<uint>>();
    const auto &i_cstr_slct = info.resource("constr_selection").getr<int>();
    ...
    if (e_vert_slct.size() > 1) {
      if (ImGui::Button("Focus selection")) {
        info.resource("viewport.input.vert", "selection").getw<std::vector<uint>>() = { i };
      }
      ImGui::Separator();
    }
    ...
    // Plot vertex settings for primary color
    {
      ...
      if (!ImGui::IsItemActive() && m_is_vert_edit_used) {
        ...
        e_appl_data.touch({
          ...
          .redo = [edit = colr_edit, i = i](auto &data) { data.verts[i].colr_i = edit; },
          .undo = [edit = m_colr_prev, i = i](auto &data) { data.verts[i].colr_i = edit; },
        });
      }

      // End the first of two groups
      ImGui::EndGroup();
      ImGui::SameLine();
      ImGui::AlignTextToFramePadding();

      // Begin the second of two groups
      ImGui::BeginGroup();
    ...
    // Plot vertex settings for mapping
    {
      ...
      if (l_csys_i != e_vert.csys_i) {
        e_appl_data.touch({
          ...
          .redo = [edit = l_csys_i,      i = i](auto &data) { data.verts[i].csys_i = edit; },
          .undo = [edit = e_vert.csys_i, i = i](auto &data) { data.verts[i].csys_i = edit; }
        });
      }
    }
    ...
    if (!e_vert.colr_j.empty()) {
      ...
      // Mapping selector
      ImGui::BeginGroup(); ImGui::SetNextItemWidth(constr_mapp_width); ImGui::Text("Color system");
      for (uint j = 0; j < e_vert.colr_j.size(); ++j) {
        ImGui::PushID(j);

        // Selector for secondary color mapping index, operating on a local copy
        uint l_csys_j = e_vert.csys_j[j];
        ...
        if (l_csys_j != e_vert.csys_j[j]) {
          e_appl_data.touch({
            ...
            .redo = [edit = l_csys_j,         i = i, j = j](auto &data) { data.verts[i].csys_j[j] = edit; },
            .undo = [edit = e_vert.csys_j[j], i = i, j = j](auto &data) { data.verts[i].csys_j[j] = edit; }
          });
        }
      ...
      for (uint j = 0; j < e_vert.colr_j.size(); ++j) {
        ...
        if (ImGui::Button(is_editing ? "Editing" : "Edit")) {
          info.resource("constr_selection").getw<int>() = j;
          info.resource("viewport.input.vert", "selection").getw<std::vector<uint>>() = { i };
        }
        ImGui::PopID();
      }
      ...
      for (uint j = 0; j < e_vert.colr_j.size(); ++j) {
        ...
        if (ImGui::ArrowButton("up", ImGuiDir_Up)) {
          e_appl_data.touch({ .name = "Swapped constraint color systems", .redo = [i = i, j = j](auto &data) {  
            std::swap(data.verts[i].colr_j[j], data.verts[i].colr_j[j - 1]);
            std::swap(data.verts[i].csys_j[j], data.verts[i].csys_j[j - 1]);
      ...
      for (uint j = 0; j < e_vert.colr_j.size(); ++j) {
        ...
        if (ImGui::Button("X")) {
          e_appl_data.touch({ .name = "Delete color constraint", .redo = [i = i, j = j](auto &data) { 
            data.verts[i].colr_j.erase(data.verts[i].colr_j.begin() + j); 
            data.verts[i].csys_j.erase(data.verts[i].csys_j.begin() + j); 
          }, .undo = [edit = e_vert,  i = i, j = j](auto &data) { data.verts[i] = edit; }});

          // Sanitize selected constraint in case this was deleted
          info.resource("constr_selection").getw<int>() = std::min(i_cstr_slct, static_cast<int>(e_vert.colr_j.size() - 1));
        }
    ...
    if (ImGui::Button("Add constraint")) {
      e_appl_data.touch({ .name = "Add color constraint", .redo = [i = i](auto &data) { 
        data.verts[i].colr_j.push_back(data.verts[i].colr_i); 
        data.verts[i].csys_j.push_back(0); 
      }, .undo = [edit = e_vert,  i = i](auto &data) { data.verts[i] = edit; }});

      // Set displayed constraint in viewport to this constraint, iff a constraint was selected
      if (i_cstr_slct != -1)
        info.resource("constr_selection").getw<int>() = e_vert.colr_j.size() - 1;
    }
  ...
  void ViewportOverlayTask::eval_overlay_color_solid(SchedulerHandle &info) {
    ...
    auto &i_lrgb_target = info.resource("lrgb_color_solid_target").getw<gl::Texture2d4f>();
    auto &i_srgb_target = info.resource("srgb_color_solid_target").getw<gl::Texture2d4f>();
  ...
  void ViewportOverlayTask::eval_overlay_plot(SchedulerHandle &info) {
    ...
    const auto &e_window    = info.global("window").getr<gl::Window>();
    const auto &e_vert_slct = info.resource("viewport.input.vert", "selection").getr<std::vector<uint>>();
    const auto &e_spec      = info.resource("gen_spectral_data", "spectra").getr<std::vector<Spec>>();
    const auto &e_appl_data = info.global("appl_data").getr<ApplicationData>();
    const auto &e_proj_data = e_appl_data.project_data;
