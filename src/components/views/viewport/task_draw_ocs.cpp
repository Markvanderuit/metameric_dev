...
namespace met {
  ...
  void ViewportDrawOCSTask::init(detail::TaskInitInfo &info) {
    ...
    auto &e_chull_wf = e_app_data.loaded_chull_wf;

    // Construct ssbo's
    m_hull_vertices = {{ .data = cnt_span<const std::byte>(e_chull.verts()) }};
    m_hull_elements = {{ .data = cnt_span<const std::byte>(e_chull.elems()) }};
    m_hull_wf_vertices = {{ .data = cnt_span<const std::byte>(e_chull_wf.verts()) }};
    m_hull_wf_elements = {{ .data = cnt_span<const std::byte>(e_chull_wf.elems()) }};
    ...
    m_hull_wf_array = {{
      .buffers = {{ .buffer = &m_hull_wf_vertices, .index = 0, .stride = sizeof(AlColr) }},
      .attribs = {{ .attrib_index = 0, .buffer_index = 0, .size = gl::VertexAttribSize::e3 }},
      .elements = &m_hull_wf_elements
    }};

    // Construct draw dispatch objects
    ...
    m_hull_wf_dispatch = { .type = gl::PrimitiveType::eLines,
                           .vertex_count = (uint) (m_hull_wf_elements.size() / sizeof(uint)),
                           .bindable_array = &m_hull_wf_array,
                           .bindable_program = &m_program };

    // Construct shader program and set non-changing uniform values
