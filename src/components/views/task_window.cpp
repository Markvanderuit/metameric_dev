...
namespace met {
  ...
  bool WindowTask::handle_save(detail::TaskEvalInfo &info) {
    ...
    if (e_app_data.project_state == ProjectSaveState::eNew) {
  ...
  void WindowTask::handle_close_safe(detail::TaskEvalInfo &info) {
    ...
    auto &e_app_data = info.get_resource<ApplicationData>(global_key, "app_data");
    if (e_app_data.project_state == ProjectSaveState::eUnsaved 
     || e_app_data.project_state == ProjectSaveState::eNew) {
  ...
  void WindowTask::eval(detail::TaskEvalInfo &info) {
    ...
    if (ImGui::BeginMainMenuBar()) {
      ...
      if (ImGui::BeginMenu("File")) {
        auto &e_app_data = info.get_resource<ApplicationData>(global_key, "app_data");
        const bool is_loaded   = e_app_data.project_state != ProjectSaveState::eUnloaded;
        const bool enable_save = e_app_data.project_state != ProjectSaveState::eSaved 
          && e_app_data.project_state != ProjectSaveState::eNew && is_loaded;
