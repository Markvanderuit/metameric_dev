...
namespace met {
  ...
  void GenObjectDataTask::eval(SchedulerHandle &info) {
    ...
    const auto &e_uplf_data   = info("scene_handler", "uplf_data").read_only<detail::RTUpliftingData>();
    const auto &e_tesselation = info(uplifting_task_name, "tesselation").read_only<AlDelaunay>();
