...
namespace met {
  ...
  void GenObjectDataTask::init(SchedulerHandle &info) {
    ...
    m_program_txtr = {{ .type       = gl::ShaderType::eCompute,
                        ...
                        .spec_const = {{ 0, true }} }};
    m_program_colr = {{ .type       = gl::ShaderType::eCompute,
                        .spirv_path = "resources/shaders/pipeline_new/gen_tesselation_weights.comp.spv",
                        .cross_path = "resources/shaders/pipeline_new/gen_tesselation_weights.comp.json",
                        .spec_const = {{ 0, false }} }};
  ...
  void GenObjectDataTask::eval(SchedulerHandle &info) {
    ...
    // Get ref. to specific program for texture or color handling
    auto &program = std::holds_alternative<uint>(e_object.diffuse)
                  ? m_program_txtr : m_program_colr;
    ...
    program.bind("b_buff_weights",     e_barycentrics.buffer());
    program.bind("b_bary_4f",          e_barycentrics.texture());
