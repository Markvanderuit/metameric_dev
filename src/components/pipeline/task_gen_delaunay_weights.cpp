...
namespace met {
  ...
  namespace detail {
    ...
    std::pair<Mesh, TreeObject<Degr>> generate_search_tree(Mesh delaunay) {
      using Tree = TreeObject<Degr>;
      ...
      fmt::print("leaf: {} - {}\n", tree.leaf_begin_i(), tree.leaf_end_i());
      ...
      for (int i = tree.leaf_begin_i(); i < tree.leaf_end_i(); ++i) { // extent of nr. of leaf objects, not nr. of leaf nodes
        ...
        tree.nodes[i] = { .b_min    = vt[0].min(vt[1].min(vt[2].min(vt[3]))).eval(),
                          .e_begin  = i_underlying,
                          .b_max    = vt[0].max(vt[1].max(vt[2].max(vt[3]))).eval(),
                          .e_extent = 1 };
      } // end for i

      // Reduce upwards levels
      for (int lvl = tree.n_levels - 1; lvl >= 0; lvl--) {
        fmt::print("level {}: {} - {}\n", lvl, tree.node_begin_i(lvl), tree.node_end_i(lvl));
